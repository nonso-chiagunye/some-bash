...
# Description: This script disables, deletes, and/or archives users on the local system.
# Usage: sudo ./disable_user.sh [-dra] USER [USERN]
...
usage() {
  ...
  echo "Usage: sudo ${0} [-dra] USER [USERN]..." >&2
...
do
  ...
  then
    ...
    # If there is no directory called $ARCHIVE_DIR 
    if [[ ! -d "${ARCHIVE_DIR}" ]]
    then
    ...
    # If there is a directory called $HOME_DIR 
    if [[ -d "${HOME_DIR}" ]]
    then
